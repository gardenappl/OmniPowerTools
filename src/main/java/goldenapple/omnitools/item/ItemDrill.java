...
public class ItemDrill extends ItemTool implements ITool {
    ...
    public MiningMode getMode(ItemStack stack) {
        if (properties.modes.length == 1 || !stack.hasTagCompound())
            return properties.modes[0];
        if (stack.getTagCompound().getByte("Mode") >= properties.modes.length)
            return properties.modes[0];
        else
            return properties.modes[stack.getTagCompound().getByte("Mode")];
    }
